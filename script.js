...
function renderProjectResults(input, result) {
  ...
  resultsEl.innerHTML = [
    ...
    renderResultItem(totalCuts, 'Quantidade total de cortes'),
  ...
  comparisonEl.innerHTML = `<details class="width-comparison"><summary><strong>Ver detalhes dos cortes</strong></summary><div style="margin-top:10px">${itemLines}</div></details>`;
...
function calculate() {
  ...
  summaryEl.textContent = currentMode === 'project' ? 'Use o botão "Copiar resumo" para compartilhar o projeto.' : lastSummary;
  copyButton.classList.remove('hidden');
}

function setMode(mode){
  currentMode = mode === 'simple' ? currentSimpleMode : mode;
  const isSheetMode = mode === 'sheet';
  const isSimpleMode = mode === 'simple';
  tabs.forEach(t=>t.classList.toggle('active',t.dataset.mode===mode));
  simpleModeSection.classList.toggle('hidden',!isSimpleMode);
  quantityGroup.classList.toggle('hidden',currentMode!=='buy' || !isSimpleMode);
  ...
  fabricLengthGroup.classList.toggle('hidden',currentMode!=='have' || !isSimpleMode);
...
document.querySelectorAll('.simple-tab').forEach(tab=>tab.addEventListener('click',()=>{
  currentSimpleMode = tab.dataset.simpleMode;
  document.querySelectorAll('.simple-tab').forEach(t=>t.classList.toggle('active', t===tab));
  if (document.querySelector('.tab[data-mode="simple"]').classList.contains('active')) setMode('simple');
}));
